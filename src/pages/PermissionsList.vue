...
// Consistent color for permission (based on libelle)
const permissionColorPalette = ['primary', 'info', 'success', 'warning', 'secondary', 'error']
const permissionColor = (libelle) => {
  if (!libelle) return 'secondary'
  let hash = 0
  for (let i = 0; i < libelle.length; i++) {
    hash = libelle.charCodeAt(i) + ((hash << 5) - hash)
  }
  return permissionColorPalette[Math.abs(hash) % permissionColorPalette.length]
}
...
const openCreateDialog = () => {
  ...
  formData.value = { idPermission: null, libellePermission: '' }
  formErrors.value = {}
  ...
  <VRow>
    <VCol cols="12">
      ...
      <div class="d-flex align-center justify-space-between mb-6 flex-wrap gap-4">
        <div>
          <h1 class="text-h4 font-weight-bold text-primary">Gestion des permissions</h1>
          <p class="text-medium-emphasis text-subtitle-1 mt-1">
            Définissez les droits d'accès granulaires de la plateforme
          </p>
        </div>
        <VBtn
          color="primary"
          ...
          <VCard variant="tonal" color="primary" class="stat-card" rounded="lg">
            <VCardText class="d-flex align-center pa-4">
              <div class="stat-icon rounded-circle bg-primary-light pa-3 me-4">
                <VIcon icon="bx-key" size="28" color="primary" />
              </div>
              <div>
          ...
          <VCard variant="tonal" color="success" class="stat-card" rounded="lg">
            <VCardText class="d-flex align-center pa-4">
              <div class="stat-icon rounded-circle bg-success-light pa-3 me-4">
                <VIcon icon="bx-check-shield" size="28" color="success" />
              </div>
              <div>
                <div class="text-caption text-uppercase font-weight-medium text-medium-emphasis">
                  Actives
          ...
          <VCard variant="tonal" color="info" class="stat-card" rounded="lg">
            <VCardText class="d-flex align-center pa-4">
              <div class="stat-icon rounded-circle bg-info-light pa-3 me-4">
                <VIcon icon="bx-lock-open" size="28" color="info" />
              </div>
              <div>
                <div class="text-caption text-uppercase font-weight-medium text-medium-emphasis">
                  Droits définis
      ...
      <VCard rounded="lg" elevation="0" class="main-card">
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="3" class="text-center pa-6">
                ...
                <div class="text-caption text-medium-emphasis mt-2">Chargement des permissions…</div>
              </td>
            </tr>
            <tr v-else-if="filteredPermissions.length === 0">
              <td colspan="3" class="text-center pa-8">
                <VIcon icon="bx-key" size="48" color="grey-lighten-1" />
                <div class="text-h6 font-weight-medium mt-2 text-medium-emphasis">
                  {{ searchQuery ? 'Aucune permission trouvée' : 'Aucune permission enregistrée' }}
                </div>
                <p class="text-caption text-medium-emphasis">
                  {{ searchQuery ? 'Ajustez votre recherche' : 'Ajoutez une nouvelle permission' }}
                </p>
              </td>
            </tr>
            <tr
            ...
            >
              ...
              <td>
                <div class="d-flex align-center">
                  <VAvatar
                    variant="tonal"
                    :color="permissionColor(permission.libellePermission)"
                    size="34"
                    rounded
                    class="me-3"
                  >
        ...
        >
          <span class="text-caption text-medium-emphasis">
            {{ filteredPermissions.length }} permission(s) — Page {{ currentPage }} / {{ totalPages || 1 }}
          </span>
          <VPagination
            v-model="currentPage"
            :length="totalPages || 1"
      ...
      <VCard rounded="lg" class="dialog-card">
        <VCardItem class="border-bottom">
          ...
          <VCardSubtitle class="mt-1 text-medium-emphasis">
            {{ isEditing ? 'Modifiez le libellé de la permission' : 'Saisissez le libellé de la nouvelle permission' }}
          </VCardSubtitle>
        </VCardItem>
